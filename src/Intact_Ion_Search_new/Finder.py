...
class Finder(object):
    ...
    '''
    def __init__(self, theoValues, configs):
    ...
    def getMz(self, mass, z):
        return mass / z + protonMass * self.configs['mode']
    ...
    def getErrorLimit(self, mz):
        return self.configs['k'] / 1000 * mz + self.configs['d']

    def findIons(self, errorLimit, flag):
        ionLists = list()
        for spectrum in self.data:
            ...
            for modif, val in self.theoValues.items():
                ...
                z = 1
                while True:
                    if self.getMz(mass,z) < self.configs['minMz']:
                        break
                    elif self.getMz(mass,z) < self.configs['maxMz']:
                        mask = np.where((abs(self.calculateError(spectrum['m/z'], self.getMz(mass,z))) < errorLimit)
                                        & (spectrum['z'] == z))
                        if (len(mask[0]) == 1):
                            ions.append(Ion(self.configs['sequName'],modif,spectrum[mask]['m/z'][0],self.getMz(mass,z),
                            ...
                            if len(mask[0]) == 0:
                                ...
                                if (len(mask[0]) == 1):
                                    ions.append(
                                        Ion(self.configs['sequName'], modif, spectrum[mask]['m/z'][0], self.getMz(mass, z),
                                            spectrum[mask]['z'][0], spectrum[mask]['relAb'][0], val[1]))
                            if len(mask[0]) > 1:
                                ...
                                if errorLimit > 6.5:
                                    ...
                                    if (len(newMask[0]) == 1):
                                        ionPicked = 1
                                        ions.append(Ion(self.configs['sequName'],modif,spectrum[newMask]['m/z'][0],
                                ...
                                if ionPicked == 0:
                                    ...
                                    ions.append(
                                        Ion(self.configs['sequName'], modif, sortedArr[-1]['m/z'], self.getMz(mass, z),
    ...
    def calibrate(self, ionLists, errorLimit):
