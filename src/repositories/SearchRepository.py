...
class SearchRepository(AbstractRepository):
    def __init__(self, ):
        super(SearchRepository, self).__init__('search.db', 'searches',
                                               ('name', "date","sequName", "charge", "fragmentation", "nrMod",
                                                "spectralData", "noiseLimit", "fragLib"), (), ())
    ...
    def getSearch(self, name):
        ...
        for ionVals in self.getItems(searchVals[0], 'ions'):
            peaks = [(peak[1], peak[2], peak[3], peak[4], peak[5]) for peak in self.getItems('peaks', ionVals[0])]
        ...
        return Search(searchVals, ions, delIons, remIons, searchedZStates)


    def getItems(self, patternId, table):
        #table = [key for key in self._itemDict.keys()][0]
        cur = self._conn.cursor()
        cur.execute("SELECT * FROM " + table + " WHERE parentId=?", (patternId,))
        return cur.fetchall()
    ...
    def insertItems(self, searchId, ions, status):
        for ion in ions:
            ionId = self.createItem('ions', ion + [status, searchId])
            for peak in ion.getPeakValues():
    ...
    def updateSearch(self, search):
        searchId = self.getSearch(search.getN)
        self.update(search.getVals(), searchId)
        self.deleteIons(searchId)
        self.insertItems(searchId, search.getIons())
