...
class TestPeriodicTableService(TestCase):
    ...
    def test_check_format_of_items(self):
        ...
        with self.assertRaises(InvalidInputException):
            self.service.checkFormatOfItems([(1,100,0.5),(2,100,0.4)],knownElements,numericals)
        with self.assertRaises(InvalidInputException):
            self.service.checkFormatOfItems([(1,100,0.5),(2,100.1,0.51)],knownElements,numericals)
        with self.assertRaises(InvalidInputException):
            self.service.checkFormatOfItems([(1,100,0.5),(1,100,0.51)],knownElements,numericals)
        with self.assertRaises(InvalidInputException):
            self.service.checkFormatOfItems([(1.1,100,0.5),(2,100,0.49)],knownElements,numericals)
